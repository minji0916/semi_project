...
//      @param directory 압축 풀 폴더
    ...
    public boolean unZip(String zipPath, String zipFileName, String zipUnzipPath, String unZipFile) {
        // 파일 정상적으로 압축이 해제가 되어는가.
        boolean isChk = false;

        // 해제할 홀더 위치를 재조정
        zipUnzipPath = zipUnzipPath + unZipFile.replace(".zip", "");
        ...
        try {
            ...
            while ((zipentry = zis.getNextEntry()) != null) {
                ...
                if (zipentry.isDirectory()) {
                    System.out.println("zipentry가 디렉토리입니다."); file.mkdirs();
                } else {
                    // 파일이면 파일 만들기
